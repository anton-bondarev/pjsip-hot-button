...
#include <local_sip_account.inc>
#include <peer_sip_account.inc>
...
static int wait_button(void) {
	...
	/* Input device - mouse. */
	if (!(dev = input_dev_lookup(INPUT_DEV_NAME))) {
		printf("Cannot find input dev \"%s\"\n", INPUT_DEV_NAME);
		exit(1);
		return -1;
	}

	res = input_dev_open(dev, NULL);
	if (res) {
		exit(1);
		return -1;
	}
	...
	while (ev.type != USER_BUTTON_PRESSED);
...
static volatile int current_call_state;
...
static void on_call_state(pjsua_call_id call_id, pjsip_event *e) {
	...
	PJ_LOG(3,(THIS_FILE, "Call %d state=%.*s", call_id,
			 ...
			 ci.state_text.ptr));
	if (PJSIP_INV_STATE_DISCONNECTED == ci.state && current_call_state == 1) {
...
int main(int argc, char *argv[]) {
	...
	for (;;) {
		pj_str_t uri = pj_str(PEER_SIP_ACOUNT);
